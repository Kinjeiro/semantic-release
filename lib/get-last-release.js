...
module.exports = ({branch, options: {tagFormat}}, {before} = {}) => {
  const tags = branch.tags
    .filter(
      (tag) => {
        console.log('ANKU , RELEASE', before, !semver.prerelease(tag.version), '\n', JSON.stringify(tag, null, 2))
        return ((branch.type === "prerelease" && tag.channels.some((channel) => isSameChannel(branch.channel, channel))) ||
  ...
  console.log('ANKU , tags', tags)

  const [{ version, gitTag, channels } = {}] = tags
